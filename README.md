...
* The Tavern should also have an email newsletter, to help create a community within the TTRPG space. 
* The website should allow the team behind the tavern to add products, amend products and delete products as demand requires. 
* The Django admin for the website should allow the team behind The Tavern to keep track of orders.
* A further fleshed-out version of this project would use additional marketing tools that would allow the team at The Tavern to understand items that are the most popular and least popular so that they can help understand inventory requirements. 
...
* Users need to be able to browse products, add them to a shopping bag and complete their purchase securely.
...
* The website should have front end functionality to allow registered staff to login and make amendments to the catalogue without having to log into the Django administration panel. 
...
* A header must include a logo and navigation bar on desktop. For mobile users, it should be possible to collapsible the navigation and the logo does not need to be seen.
...
Users should receive messages to let them know of any state changes. These messages should fade after 4 seconds but also have a X to manually close the message. 
...
The navigation bar should feature the following links: Home, Products (which should then be broken down into two categories: Dice, which is further split down into three subcategories for the material of the dice, Table Accessories, All Products) and About. The profile and orders links should be available in a different section to the navigation bar. 

#### User and Superuser Flow
...
### Surface
#### Colour Palette
As The Tavern is a website for TTRPG/D&D items, the website should have a rich, luxurious fantasy feel, like to the aesthetics seen in games like Baldur's Gate and other D&D based games. I used Coolors to help create the initial colour palette, which is focused on the richness and warmth of an adventuring party, without leaning into the reds and golds that you often see associated with D&D and TTRPG games.
...
At the end of the development of the website I undertook WAVE testing, which raised major contrast issues with the current colour palette. With that in mind, the primary accent was tweaked slightly to make it darker and pass the WAVE minimum checker, whilst keeping the original colour palette in mind. The refined palette can be seen below. It should be noted that WAVE did find a further 2 contrast errors, but as these were so minor they were left as they were:
...
Using the Agile Methodology, I first created a set of epics, which were then broken down into a series of user stories to better understand the requirements of the website. These user stories were written in the following formation: As a *Role* I can *Capability* so that *Receive Benefit*.

### Epics
#### Products
* Users are able to browse all the products available.
* Users can learn the details of individual products, including materials and dimensions per item.
* Users can view reviews of products.
Registered users can leave reviews of products.
* Users can see when they have put a product into their shopping bag.
* Superuser can add, remove and delete products.
* Superuser can to keep product details up to date.

#### User Account and Authentication
* Users can sign up to become registered users
* Users can receive a confirmation email once they have completed a product purchase
* Registered users can securely log in and log out of their account
* Registered users can manage their own personal details in their profile, including a profile picture
* Registered users can reset their password at any time
* Registered users can see past orders attached to their account
...
#### User Account and Authentication
* As a user, I can sign up to become a registered user, so that I can have a profile that tracks previous orders and automatically populate fields with my details at checkout.
* As a user, I will receive a confirmation email when I have created an account, so that I can securely validate my account.
* As a registered user I can securely log in and log out of my profile so that I know that my account is safe.
* As a registered user, I can update my profile information, so that I can be sure that my details are up to date.
* As a registered user, I can give myself a profile picture, so that I can have a photo on my profile.
* As a registered user, I can reset my password at any time, so that I can keep my account secure.
* As a registered user, I can see my previous orders, so that I can track any orders I have made in the past. 
...
Using the MoSCoW prioritisation method, I then further broke down my user stories into four separate categories. These categories are:
...
* As a user of the website, I can change the quantity of the items in my shopping bag and an updated grand total, so that I can track how much I am spending
* As a user of the website I can remove items from my shopping bag and see an updated grand total, so that I can remove items I do not need anymore
* As a user of the website I can be shown all the items I am buying, plus the subtotal, shipping details and grand total before I complete the purchase, so that I can decide that I want to complete the purchase
* As a user of the website I can enter my details into the checkout securely, using the Stripe API, so that I can purchase items safely and securely
* As a user, I want to be able to sign up to The Tavern's newsletter, so I can learn about their community
...
## Website Features
### Header
The header extends the base.html template, and is a simple, minimalistic design which is visually appealing for users. On mobile the header is much simpler, displaying only the most crucial features of the website to allow users easy UX - these features are: a drop-down burger icon which allows users to navigate to the following: all products, dice, other accessories, about us, Frequently Asked Questions (FAQ) and contact us. There is also a search button, allowing users to search the site, a my account button, and a basket button. On tablets and up, the header is much more elaborate, featuring a small version of the company's logo on the left-hand side, a central search bar and the account and basket features on the right-hand side of the screen. Running just below this in a separate bar is the product catalogue, and separated from this the FAQ and the contact us button can be found in the header. This allows users to easily navigate to the product directory, but requires them to search a little further for the other pages, which is the goal of an eCommerce site. 

### Newsletter Sign Up
The newsletter sign up extends from base.html and acts as part of an elongated footer. This is connected to Mailchimp, and once the user has provided their email address this is recorded in Mailchimp's database. On providing the email, the user is shown a success message confirming that their information has been collected. 

### Footer
The footer also extends from base.html and is very simple, made up of a few links for users to navigate around the site, and to find the social media links for the company. All the social media links apply the "rel=noopener" rule, and open to a new page away from the site. The footer is responsive to various screen sizes, stacking on mobiles, and stretching out from tablet onwards. 
...
The testimonials section is made up of another simple model, allowing staff to update the testimonials shown on the website. The testimonial is made up of a quote, a name, and a tag line (often the typical character a player goes by). Superusers can update any testimonial, as well as add, delete and change the order of the testimonials. On a mobile screen, testimonials stack. 
...
The product catalogue is designed mobile first, and products stack vertically on a mobile, rows of two on a tablet and rows of four on desktop and above. There is a small arrow that floats on the right hand side of the items which takes users from to the top of the page using a small chunk of JavaScript. This code was inspired by the Boutique Ado walkthrough and adjusted for the project needs. 

### Product Details Page
Upon clicking any of the items on the product catalog, the user is taken to the individual product description page. Products are searched for using their slug, rather than their individual ID number. This results in better UX for users as they can understand the product they are searching for much easier than if they were required to know the product number. The product details page includes the following - the product tag, a product image, a product title, a product description, a product price, a product quantity toggler and a dimensions and materials drop down accordion. When accessed via mobile, the columns automatically vertically but on tablet and larger screens, the columns split into two showing the product image on the left-hand side and the product details on the right. 

As dice can be sold either individually or as part of a seven-piece set, dice have an additional option where users can choose whether they are buying the individual price, or the full set, which affects the price shown on the site. Dice products will automatically default to the single D20 price when the user loads onto the website. To determine whether the product is a flat price or part of the single vs set cost, the below model was created: 
...
On putting an item in the shopping bag, the website utilises Bootstrap's built-in toast system to indicate to the user that the item has been added to their bag. The toast has a X for users to close but will also automatically fade after 5 seconds. Within the toast, the user can see the item that has been placed into the bag, the quantity of the item and the cost as well as see if they have put enough things in their shopping bag to get free delivery. Finally, the user is shown a button which takes them to the shopping bag page. 

Registered Users can also leave product reviews on items they have purchased. Reviews are stored in the ProductReview model, which is linked to the Product and User models ensuring that reviews can be connected to individual products and users. Only authenticated users can submit a review. If a user attempts to leave feedback while logged out, they are redirected to the login page. Submitted reviews do not appear immediately on the site; instead, they are set to a “pending” status by default. Admin users can then approve or reject reviews through the Django admin panel. This moderation step ensures that inappropriate or spam content does not appear publicly. Once approved, reviews are displayed on the relevant product detail page, showing the review title, written feedback, star rating, and associated user information. If a user edits an approved review, it is automatically returned to a pending state and must be re-approved before becoming visible again. This maintains quality control even after initial moderation.

Where multiple reviews exist for a product, the star ratings are aggregated and displayed to give users a quick visual overview of overall customer satisfaction. This feature adds transparency, builds trust, and strengthens the overall shopping experience by incorporating community feedback directly into the product pages.
...
### Shopping Bag
The shopping bag, or the Bag of Holding as it is called on the website in line with the D&D theme, is the next step in the purchasing process for a user and should give the user an immediate overview of the products they are in the process of purchasing. The page is designed to be as simple as possible, with a table which includes the product image, the item that is being purchased, the quantity and the subtotal. Underneath this, the user can see the bag total, delivery costs and the grand total. If the user has not reached the threshold for free delivery, they are informed how much more they need to spend to reach the free checkout threshold. Within the table, users can amend their product quantity, increasing or reducing it (no lower than 1) or removing the entire item from the bag. To wire up the quantity and remove buttons and make them functional, a small amount of JavaScript was written, inspired by the Boutique Ado code: 
...
The code has two versions - a mobile version and a desktop version, which changes dependent on how the user is accessing the bag. On mobile the table cannot be seen and all of the items stack vertically into readable columns. The buttons are still accessible, allowing users to increase or decrease quantity, and the bin button can be used to remove the item from the bag. Provided the user is happy with everything, they can proceed to the next stage of the secure checkout via a button. If they would like to go back and add more items to their shopping bag, there is another button which returns them to the product catalog. 

### Checkout
The checkout page allows users to securely complete their purchase using Stripe’s integrated payment system. The layout is fully responsive, displaying the order summary alongside the delivery form on larger screens and stacking content into a column on mobiles to maintain clarity and ease of use. Required fields are validated before submission to prevent incomplete orders, and Stripe Elements is used to securely capture card details without sensitive payment data ever passing through or being stored on the application’s server. Real-time validation messaging is provided directly by Stripe for incorrect or incomplete card details:
 ...
 While the transaction is being processed, a loading spinner appears on the “Complete Order” button to prevent duplicate submissions and provide clear feedback to the user.
...
To ensure reliability and data integrity, Stripe Webhooks are implemented to confirm payment success independently of the frontend. When a payment is successfully processed, Stripe sends a payment_intent.succeeded event to the application’s webhook endpoint. The webhook handler verifies the event using the Stripe webhook secret before securely creating the order in the database (or confirming it if it already exists). This server-side confirmation ensures that orders are recorded even if the user closes their browser, loses connection, or fails to reach the success page. 

Once the webhook successfully confirms the payment, the order is stored with its associated user details, delivery information, and line items. A confirmation email is then triggered to provide the user with a record of their purchase. This webhook-driven approach adds an additional layer of robustness to the checkout flow, ensuring that payment confirmation and order creation are handled securely and reliably at the server level rather than relying solely on the website and its redirects.
...
The Order Confirmation page provides users with clear confirmation that their purchase has been successfully processed. Once payment has been securely verified via Stripe and confirmed through the webhook handler, users are redirected to this page where a full summary of their order is displayed. This includes the order number, date, delivery details, billing information, individual line items purchased, subtotal, delivery cost, and grand total. This information is stored to the user's profile and can be restored by the user going to their profile and clicking on the order number to open the confirmation. 

The confirmation page pulls through data stored in the database following successful webhook validation. Information that is updated in the Django Admin panel (if for example a customer called up to manually make a change to their order), appears on the final order history. As part of the completion process, the session shopping bag is cleared to prevent duplicate orders or confusion. Registered users can later revisit their order confirmation details via their profile page, where a full order history is maintained. 

### Profile
The Profile page provides registered users with a personalised account area where they can manage their stored information and review their order history. The layout is fully responsive, stacking into a singular column on mobiles and displaying structured columns on tablets and larger screens for improved readability and organisation. The page is divided into clearly defined sections, including a profile image, default delivery details, and a complete history of previous transactions.

The profile functionality is built using a UserProfile model, which links to Django’s built-in User model using a one-to-one relationship. When a user updates their delivery details on their profile, the information is saved to their linked UserProfile. These saved details are then used to automatically populate the checkout form the next time they place an order, making the process quicker and easier for returning users.
...
Order history is handled through a relationship between the Order model and the UserProfile model. Each order includes a ForeignKey linking it to the correct user profile (using related_name="orders"), which ensures that every purchase is tied to the right account. This link enables the profile page to pull in and display all previous orders for that user automatically. Each order in the list links back to its confirmation page, allowing users to review what they bought, their delivery details, and the total amount paid.

Profile images can be uploaded and stored via the model’s image field, with a default image automatically assigned where none is provided. 
...
### Contact Form
The contact form gives users a straightforward way to message The Tavern team directly from the website. Messages are saved into the database via the ContactForm model when the form is submitted, so staff can view and manage them securely through the Django admin panel. The form uses Django’s built-in validation to ensure required fields are completed (such as name, email, and topic), and on successful submission the user receives an on-screen toast confirming their message has been sent.
...
| --- | --- |
Edge 145 | No issues
Firefox 147 | No issues
Google Chrome 145 | No issues
Opera 127 | No issues
...
| --- | --- | --- | --- |
Header Bar | Header Bar is responsive to device | On tablets and desktops the header bar should show in full, stretching out across the full page with the logo on the left-hand side, the search bar in the centre and the account icon and basket icon on the right-hand side. On mobiles, this bar should not be seen at all, but the user sees the burger icon, the search icon, the account icon and the basket icon. On clicking the search icon, the search bar drops down and appears | On tablets and desktops the header bar shows in full, stretching out across the full page with the logo on the left-hand side, the search bar in the centre and the account icon and basket icon on the right-hand side. On mobiles, this bar cannot be seen at all, but the user sees the burger icon, the search icon, the account icon and the basket icon. On clicking the search icon, the search bar drops down and appears
...
Testimonials | Testimonials should render and should show accurate testimonials as set up in the Django Admin | Testimonials should render as the following - a small paragraph with the testimonial and underneath that, the name of the person providing a testimonial, the class/race they typically play, and their location as a sub header. The testimonial that shows should match the information that has been set in the Django Admin, including the order set on the admin | Testimonials render as the following - a small paragraph with the testimonial and underneath that, the name of the person providing a testimonial, the class/race they typically play, and their location as a subheader. The testimonial that shows matches the information that has been set in the Django Admin, including the order set on the admin

#### Products
| Feature Tested | Action | Expected Result | Actual Result |
| --- | --- | --- | --- |
Products Page | Products page renders showing the product catalogue and is responsive | The product page should render when loaded and display the catalogue. It should be responsive to the device used, stacking into a single column on mobiles, and stretching out into four columns per row on desktops. | The product page renders when loaded and displays the catalogue. It is responsive to the device used, stacking into a single column on mobiles, and stretching out into four columns per row on desktops.
...
Product Page | Edit and Delete buttons show only when the Superuser is logged in | The Edit and Delete buttons which launch the product amendment pages or delete the product from the catalogue should only be visible for Superuser. Normal users should not see these buttons at all | The Edit and Delete buttons which launch the product amendment pages or delete the product from the catalogue are only visible for Superuser. Normal users do not see these buttons at all
...
Product Details | Edit and Delete buttons show only when the Superuser is logged in | The Edit and Delete buttons which launch the product amendment pages or delete the product from the catalogue should only be visible for Superusers. Normal users should not see these buttons at all | The Edit and Delete buttons which launch the product amendment pages or delete the product from the catalogue are only visible for Superusers. Normal users do not see these buttons at all
Product Details | Reviews - User must be logged in | Users should be logged in to provide a review. If the user is logged out, they should be sent to the login page. | Users must be logged in to provide a review. If the user is logged out, they are sent to the login page.
...
Bag | Product quantity increase and decrease button works and updates quantity in real time | The product quantity increase and decrease buttons should update when being clicked by a user. The quantity number should update as this goes up and down in real time and should the user add the item to their bag, the quantity should match the quantity in the bag | The product quantity increase and decrease buttons updates when clicked by a user. The quantity number updates as this goes up and down in real time and should the user add the item to their bag, the quantity matches the quantity in the bag
...
Bag | Keep Shopping and Secure Checkout buttons work | The "Keep Shopping" button should return the user to the product catalogue, the "Secure Checkout" should send the user onto the checkout page | The "Keep Shopping" button returns the user to the product catalogue, the "Secure Checkout" sends the user onto the checkout page
...
Checkout Confirmation | "Continue Shopping" button | "Continue Shopping button should return users to the products catalogue | "Continue Shopping" button returns users to the product catalogue
...
Privacy Policy | Privacy Policy renders | The Privacy Policy should render and show the full policy | The Privacy Policy renders and shows the full policy
...
Shipping | Shipping renders | Shipping should render and show all information | Shipping renders and shows all information
Returns | Returns renders | Returns should render and show all information | Returns renders and shows all information
...
As a user, I can sign up to become a registered user, so that I can have a profile that tracks previous orders and automatically populate fields with my details at checkout | Must Have | Met
...
At the beginning of the project, I sat down and documented all the keywords associated with The Tavern, to try and improve my SEO rating as much as possible. To start this process, I began by noting down all the general keywords associated with my website:
...
* Clear signposted opportunities to sign up to the mailing list
...
Were the project to be developed further, I would strongly suggest that The Tavern move to launch platforms on TikTok and Instagram, which is why those two platforms have been linked in the footer. As TTRPGs grow in media presence due to platforms like Critical Role and Dimension 20, the audience is getting younger and tech savvier, and these platforms are better suited to that environment. TikTok and Instagram work more with short content often called "reels", which capture an audience that is increasingly scrolling on the go. I would suggest that the company focus on building a sense of community within these platforms, offering followers tips, tricks and suggestions for them to incorporate into their home campaign, rather than solely focusing on using social media as a further tool to sell the product. Customers are increasingly suspicious of a company just selling to them and therefore becoming an expert and trusted source for TTRPG information will build the trust that will make it easier to bridge that gap from follower to purchaser. 

### Email Marketing
Within the project I have implemented a mailing list sign up which sits just above the footer and appears on every page. This will allow the team at The Tavern to create and build a newsletter community and create brand awareness. As the project only required me to create the newsletter, I have gone no further in developing this. However, if the team were to further develop this, I would encourage a weekly newsletter, which includes promotions, however I would strongly advise against using the email marketing newsletter as a pure sales tool. Again, I would suggest that The Tavern work to build a community, so the newsletter becomes a helpful source of tips and tricks for improving gameplay as well as fun "race/class of the month", then it will be much easier to drip feed in some occasional product launches, rather than just inundating users with promotions and products. 
...
4. Make sure to uncheck "Block all public access" option to allow public read access to your files.
5. Enable static web hosting on the bucket.
6. Permission - Add a bucket policy to allow public read access to objects in the bucket. 
7. Create an IAM user with programmatic access and attach a policy that grants the necessary permissions to access the S3 bucket:
    ...
    4. Attach a policy that allows S3 access to your bucket.
...
5. Open your deployed application in the browser:
