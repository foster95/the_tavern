...
* A further fleshed-out version of this project would use additional marketing tools that would allow the team at The Tavern to understand items that are most popular and least popular so that they can help understand inventory requirements. 
...
* A header must include a logo and navigation bar on desktop. For mobile users, this navigation bar should be collapsible and the logo does not need to be seen
...
### Surface
#### Colour Palette
As The Tavern is a website for TTRPG/D&D items, the website should have a rich, luxurious fantasy feel, similar to the aesthetics seen in games like Baldur's Gate and other D&D based games. I used coolors to help create the initial colour palette, which is focused on the richness and warmth of an adventuring party, without leaning into the more cliche reds and golds that you often see associated with D&D and TTRPG games.
...
At the end of the development of the website I undertook WAVE testing, which raised major contrast issues with the current colour palette. With that in mind, the primary accent was tweaked slightly to make it darker and pass the WAVE minimum checker, whilst still keeping the original colour palette in mind. The refined palette can be seen below. It should be noted that WAVE did find a further 2 contrast errors, but as these were so minor they were left as they were:
...
The profile functionality is built using a UserProfile model, which links to Django’s built-in User model using a one-to-one relationship. This means extra information — such as default delivery details and a profile image — can be stored separately without changing Django’s core authentication system. When a user updates their delivery details on their profile, the information is saved to their linked UserProfile. These saved details are then used to automatically populate the checkout form the next time they place an order, making the process quicker and easier for returning users.
...
Order history is handled through a relationship between the Order model and the UserProfile model. Each order includes a ForeignKey linking it to the correct user profile (using related_name="orders"), which ensures that every purchase is tied to the right account. Because of this link, the profile page can pull in and display all previous orders for that user automatically. Each order in the list links back to its confirmation page, allowing users to review what they bought, their delivery details, and the total amount paid.
...
Part of this issue was resolved quickly after I realised that there were a number of issues and inconsistencies with my env credentials and in the settings.py folder for email settings, which I fixed. On fixing this, the emails did begin to send, however they began to send in duplicate, particularly when the website was running slowly. To fix this, I added the following to the order model:

            confirmation_email_sent = models.BooleanField(default=False)

After running the standard migrations required for the model, I updated the webhook logic which searched for the confirmation email sent boolean and decides whether or not to send an email. Further, this webhook acted so if it sent the email, it would automatically toggle the boolean from False to True, which wouldn't allow for duplicate emails. 
...
#### Sales and Discounts? 
As The Tavern is a small business, I would only advocate that they do sales seasonally rather than all the time, or in line with important events within the D&D community that The Tavern could piggyback off, such as season finales of Dimension 20 or Critical Role. 

#### Goals of the Business
The ultimate goal of The Tavern is to sell D&D accessories, but also to build a community of D&D and TTRPG fans

#### Budget
The Tavern does have a small marketing budget, however I would personally suggest to them that the rely on organic marketing rather than paid for marketing. You can often find that it is the outsiders and the misfits that drift towards D&D and TTRPG and those people can be especially cynical and shrewd about paid for marketing. After all reputation takes a lifetime to build and seconds to shatter. 
